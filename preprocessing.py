...
label_colours = [(0, 0, 0),  # 0=background
                 # 1=solidline, 2=dashline, 3=arrow, 4=otherline, 5=stopline, 6=backpoints
                 (220, 0, 0), (0, 220, 0), (220, 220, 0), (0, 0, 220), (220, 0, 220),(220,220,220)]
...
def random_rescale_image_and_label(image,label, min_scale, max_scale,rd_scale):
...
def random_crop_or_pad_image_and_label(image, label, crop_height, crop_width, ignore_label,cp_x,cp_y,rd_scale):
  ...
  c_heihgt = math.floor(crop_height*rd_scale)
  c_width = math.floor(crop_width*rd_scale)
  image_and_label_crop = tf.cast(tf.image.crop_to_bounding_box(
      image_and_label_pad, cp_x, cp_y, crop_height, crop_width ),dtype=tf.float32)
  """ image_and_label_crop = tf.random_crop(
      image_and_label_pad, [crop_height, crop_width, 5]) """

  image_crop = image_and_label_crop[:, :, :4]
  label_crop = image_and_label_crop[:, :, 4:]
...
def eval_input_fn(image_filenames,region_filenames, label_filenames=None, batch_size=4):
  ...
  def _parse_function(filename, is_label):
    if not is_label:
      image_filename, region_filename = filename
      label_filename = None
    else:
      image_filename, region_filename, label_filename = filename

    image_string = tf.read_file(image_filename)
    image = tf.image.decode_image(image_string)
    image = tf.to_float(tf.image.convert_image_dtype(image, dtype=tf.uint8))
    image.set_shape([321, 321, 3])

    region_string = tf.read_file(region_filename)
    region = tf.image.decode_image(region_string,channels=1)
    region = tf.to_float(tf.image.convert_image_dtype(region, dtype=tf.uint8))
    region.set_shape([321, 321, 1])
    ...
    if not is_label:
      ...
      label = tf.image.decode_image(label_string,channels=1)
      label = tf.to_int32(tf.image.convert_image_dtype(label, dtype=tf.uint8))
      label.set_shape([321, 321, 1])

      return image, label

  if label_filenames is None:
    input_filenames = (image_filenames, region_filenames)
  else:
    input_filenames = (image_filenames, region_filenames, label_filenames)

  dataset = tf.data.Dataset.from_tensor_slices(input_filenames)
  if label_filenames is None:
    dataset = dataset.map(lambda x, y: _parse_function((x, y), False))
  else:
